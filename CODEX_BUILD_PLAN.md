...
- An explicit home-surface decision at this checkpoint's design gate: whether the application opens on the Shelf or on the recommendation. A shelf-first product reads as a reading tracker regardless of positioning, because the first screen is the product claim — a first-time viewer of the Checkpoint 7P preview said it "looks like Goodreads," which was accurate while no recommendation existed. Shelf-first was correct when there was nothing else to show and stops being correct once there is. See `docs/product/differentiation.md`. Retest the comparison on a fresh viewer after the first bag ships; if it still reads as a tracker, the differentiation has not landed.
