...
2. **Per-child content-based statistical learning.** A small model learns which *already-known* book characteristics predict outcomes for one child — regularized or Bayesian logistic regression, a simple pairwise preference model, or similarly low-parameter methods. Crucially, it does not learn what books *are*: it consumes a representation built from verified external metadata and learns only this child's preference function over that representation. That is a far smaller statistical problem than the tier below, and it can become useful with tens of good observations rather than millions.
3. **Population / collaborative learning** (what Netflix, Amazon, and YouTube actually run): collaborative filtering, matrix factorization, hierarchical population models, and deep recommenders *learn latent parameters* by finding patterns across many users. Amazon's item-to-item collaborative filtering and Netflix's factorization approach are the canonical examples.
4. **Language-model-based**: an LLM generates, ranks, or explains recommendations from text. Kept separate because its privacy, truth, latency, and cost risks differ in kind from classical statistical learning.

**An earlier version of this section collapsed tiers 2 and 3 into one and concluded that all learned ranking needs Netflix-scale data. That inference was too broad and is corrected here.** It is true of tier 3 and false of tier 2. Tier 3 needs millions of users because it is estimating a shared latent space from interactions; a single household, or even a hundred-household beta, cannot support it, and the cold-start literature (see Further reading) is specifically about that failure. Tier 2 is a different problem: the book representation comes from verified metadata that already exists, and the only thing being fit is one child's preference function over it.

**A second correction, about risk.** A classical learned ranker structurally confined to scoring already-verified candidate IDs using already-approved features **cannot fabricate.** It cannot invent a book, an availability claim, an age band, or a reading history, because it never produces any of those — it produces an ordering over candidates that already passed eligibility. Its real risks are overfitting, poor calibration, opacity, instability under sparse evidence, and learning spurious correlations. Those are serious and are why it stays gated, but they are not the fabrication risk that governs tier 4, and conflating them suppressed a legitimate option for the wrong reason.

Tier 1 remains the V0.1 production champion. Tier 2 becomes evaluable at Gate G7 (section 6.4.4) and implementable only under a separately approved Checkpoint 11B. Tier 3 requires cross-household data and its own privacy and data-governance basis, and is not in scope for V0.1 or for 11B.
...
3. Per-child learned ranking (tier 2). A small model adjusts the score for one child, over features derived from already-verified metadata. It carries a real opacity and overfitting cost but not a fabrication cost, because it never creates candidates or facts. It is evaluated at Gate G7 and, if it earns promotion, implemented only as an ensemble term alongside the deterministic scorer under Checkpoint 11B — never as a wholesale replacement. Section 6.5 specifies it.
4. Population learning (tier 3). Requires many households, a separately approved privacy and data-governance basis, and demonstrated benefit over per-household learning. Explicitly out of scope for V0.1 and for Checkpoint 11B.
...
The recommender-systems reviewer persona in `AGENTS.md` is the standing lens for this section. It reviews the deterministic weights when they are set at Checkpoint 7D and leads the diagnosis at Checkpoint 11A, so the question "are these picks actually good" is answered by someone who has tuned real ranking systems rather than inferred from the absence of test failures.
...
- **G3 — bake-off.** The major pivot gate. Leave-one-positive-out recall for three competing methods plus owner judgment on at least three bags each. **Four outcomes, so the gate has a stated PASS and not only failure conditions:**
  - Trivial baseline beats V0.1 substantially → **build Phase C.**
  - Frontier language model badly beats the trivial baseline → **P1.**
  - Frontier language model roughly equals the trivial baseline → **build Phase C.** A tie goes to the deterministic method: it is free at query time and structurally cannot fabricate, so equal quality is a win for it, not a coin flip.
  - Nothing beats V0.1 → **P6.**

  Four constraints on how it is run, because the naive version measures the wrong thing:
...
- **G7 — personal learning value.** Scored inside Checkpoint 11A, triggered by accumulated evidence rather than elapsed time. Compares the deterministic champion against a per-child learned challenger and a content-similarity baseline, prequentially. New-creator delight rate is a **veto**, not a weighted term. Specified in section 6.5.7. A "roughly equivalent" result is the expected outcome at small sample sizes and keeps the champion.
...
### 6.5 The personal learning layer — planned, gated, not authorized

**Status: planning only.** Nothing in this section authorizes a model, a dependency, a schema change, a migration, or any change to production ranking. It exists so that the evidence needed later is captured now, and so the option is decided on measurement rather than on enthusiasm or on the overly broad claim corrected in section 6.3.

**The product thesis, stated as a product rather than a technology.** Bookkin learns what works for this child, and what works when this caregiver and this child read together. Machine learning is a possible means to that; it is never the promise. The consumer-facing claim is the effect — recommendations get more personal because Bookkin learns from what actually happens when your family reads — and any public claim of "machine-learning personalization" is forbidden until a learned model is actually running in production under an approved checkpoint. Until then, claims describe deterministic learning from history without implying a deployed model.

#### 6.5.1 What is preserved unconditionally

The learning layer is built on top of the existing guarantees, never instead of them: verified candidates only; catalog truth boundaries; deterministic V0.1 as the champion; child and caregiver reactions separate and never averaged; events separate from reactions; rereads as strong positive evidence and their absence as neutral; explicit negatives only when truthfully observed; confidence-ramped personalization; current interests distinct from historical; illustrator separate from author; inverse-frequency heading weights; both composition constraints from Checkpoint 7D; at most one bounded exploration pick; quality and suitability gates applied *before* personalization; embeddings computed at corpus construction but not automatically admitted to ranking; delight rate and new-creator delight rate as the success measures; human approval gates; and no household history to an external model without a separate privacy gate.

#### 6.5.2 Three distinct questions, not one model

- **Child preference:** how likely is this child to delight in this book? Fed by child love, like, explicit dislike, stopped reads, rereads and repeated rereads, current interests, and durable taste evidence.
- **Caregiver read-aloud fit:** how well will this book work for the adult doing the reading? Fed by separately recorded caregiver reactions and, only if later approved, read-aloud characteristics such as text burden, repetition tolerance, and length. **These are never inferred when the evidence does not exist.**
- **Shared-reading fit:** how likely is this to be a good experience for the two of them together? Initially **not** a third trained model. Estimate child fit, estimate caregiver fit where evidence exists, and combine through an inspectable composition policy. A separately trained dyad model requires evidence of enough joint outcomes to justify it.

**Caregiver fit is the strongest product idea here and the weakest data position, and the plan should not pretend otherwise.** Caregiver reactions are optional and will stay sparse. But adult preferences are stable, verbalizable, and cheap to *ask for* — "no rhyme, nothing over ten minutes" is thirty seconds of input that remains true for years. Prefer direct elicitation over inferring from sparse reactions, and treat learned caregiver fit as a later refinement rather than the first route.

The V0.1 identity limitation stands: unless caregivers are individually identified, the system knows the child's relationship to household caregiver evidence, not a permanently identified person. A person-specific dyad requires its own identity and privacy decision.

#### 6.5.3 Book representation before preference

The model must not learn what books are from twenty reactions. A versioned per-work representation — conceptually `BookFeatureVectorV1`, not implemented here — is derived from approved, source-provenanced inputs only: age and suitability band, tone headings and their inverse-frequency weights, topic codes, current-interest match, author, illustrator, creator familiarity, holdings consensus, curation-list membership, award and quality signals, approved tone-vector features, the summary embeddings already computed during the Checkpoint 7B corpus pass, and explicit missingness.

**Raw high-dimensional embeddings do not enter a small-data learner directly.** Prefer a handful of low-dimensional derived features: similarity to the child's positive centroid, similarity to explicit negatives, weighted heading overlap, illustrator affinity, author affinity, current-interest match, quality score, discovery status. Every vector is versioned, reproducible, source-provenanced, explicit about missing values, and structurally unable to introduce a book fact.

#### 6.5.4 Cold start is a product feature, not a waiting room

Recommendations are useful immediately and personalization deepens with evidence. **No minimum number of rated books ever blocks a recommendation.**

- **Starter Bag.** For a child with no taste outcomes, build from age and suitability, reading context, any declared interests, professional curation evidence, and deliberately broad feature coverage. These must be excellent books even if Bookkin learns nothing from them, and they are never presented as deeply personalized.
- **Optional onboarding calibration.** Offer a small, deliberately diverse set of excellent age-appropriate books. Truthful responses only — loved it, liked it, not for us, **have not read it**, the last producing no taste evidence. Never required, never completed under pressure. Books the family has not read become good Starter Bag candidates, so the exercise pays off either way.
- **Internal evidence stages, never a progress meter shown to caregivers.** Zero to four clear outcomes: curated and contextual, household evidence weak. Five to nine: meaningful deterministic personalization; a learned model may train in shadow. Ten to nineteen: challenger evaluated offline against the champion. Twenty or more mature unique-work outcomes: eligible for a separately approved live experiment. These are engineering hypotheses to be tuned from evidence, not thresholds with scientific standing.

#### 6.5.5 The first learned problem, deliberately narrow

Given this candidate's approved feature vector and this child's truthful history, what is the probability the child responds positively? Candidate families for later evaluation are L2-regularized logistic regression, Bayesian logistic regression, or a similarly small interpretable probabilistic model. No library, framework, or algorithm is selected by this section.

**Labels.** Strong positive: `child_love`, a reread, repeated rereads as greater confidence. Moderate positive: `child_like`. Negative: explicit `child_dislike`, stopped-and-disliked, and other explicitly observed negatives the domain contract approves. **Neutral and unlabeled:** read once without a reread, no realistic exposure, catalog open only, saved only, "already read it" with no remembered reaction, unknown historical reaction. Absence is never converted into a negative label.

**Count independent evidence honestly.** Train on unique works, not raw events. Five rereads of one book raise confidence that *that book* is loved; they are not five books and must never make the training set look five times larger.

#### 6.5.6 Four constraints that the naive version of this gets wrong

- **New-creator delight rate is a veto at G7, not a term to trade off.** A model maximizing the probability of a positive reaction is conservative by construction, because the confirmed positives are books resembling ones already loved. It can therefore beat the champion on aggregate delight rate while quietly gutting discovery — the same failure G6 exists to catch in the human-tuning case, running faster and without a person noticing. A challenger that wins on aggregate and loses on new-creator delight rate is **rejected**, not averaged. Related: the composition quota is a floor that becomes a ceiling under optimization pressure, producing exactly-minimum discovery in every bag, so the quota alone is not sufficient protection.
- **The champion is not a fixed target, and comparing against it naively is unfair to the challenger.** The deterministic weights are hand-tuned by the owner across Phase D using the same outcomes — human learning from the same data, including what would be the challenger's test period. Prequential evaluation of a challenger against a champion that has seen the future is biased. **Therefore weight versions must carry effective dates from Checkpoint 7D onward**, so the champion can be replayed as of each cutoff. This costs nothing now and cannot be reconstructed later.
- **Roughly twenty outcomes will almost certainly not be enough, and that is the expected result.** With about twenty outcomes and perhaps a dozen positives, standard guidance of five to ten events per predictor supports one or two features, not the eight listed above. Correct regularization shrinks the model toward its prior, and the sensible prior is the deterministic scorer — so a properly built model at that size will approximate the champion. Realistic differentiation needs closer to sixty-five to a hundred and thirty outcomes, which at one library trip a fortnight is roughly **one to two years**. Stating this prevents a correct "roughly equivalent" result from being read as failure, and prevents anyone tuning toward a premature pass.
- **Exploration stays deterministically driven until uncertainty is measured.** Section 6.5.8's active learning needs trustworthy model uncertainty, which needs diverse evidence, which is what exploration produces — a circle. Exploration selection therefore remains governed by the deterministic diversity constraints until a calibration check passes at a stated sample size, and calibration is not meaningfully measurable at twenty outcomes.

#### 6.5.7 Reproducibility, champion/challenger, and Gate G7

A learned model does not get to destroy inspectability. Any model has a reproducible snapshot recording the child or profile, model family and version, feature-schema version, training cutoff, provenance of every training example, hyperparameters, learned parameters, evaluation results, and champion/challenger status. **No request-time online updates.** Corrections and retractions are excluded from later training exactly as they are excluded from deterministic taste evidence. Any past recommendation stays explainable in terms of the snapshot that produced it. Schema and persistence require their own architecture and data-design gate.

**Evaluation is prequential:** train only on evidence available before outcome N, predict before outcome N, record the actual result, advance the cutoff. Random train/test splitting leaks future reactions into past predictions and is not permitted. Offline metrics — calibration, ranking diagnostics — are secondary and diagnostic. **Promotion is decided on real product metrics:** delight rate, new-creator delight rate as a veto, discovery quality, shared-reading fit where measurable, and recommendation usefulness. A model that improves an offline number and not a real bag has not earned anything.

**Gate G7 — personal learning value**, scored within Checkpoint 11A. Triggered by evidence, not by the calendar: enough mature, high-confidence unique-work outcomes, with both positive and negative information present. Compare the deterministic champion, the per-child learned challenger, a content-similarity baseline, and any other explicitly approved challenger. Outcomes: **materially better** proposes a separately approved Checkpoint 11B and never activates silently; **roughly equivalent** keeps the champion, because the learned system has not earned production complexity; **worse** rejects or redesigns and never responds by adding model complexity; **insufficient evidence** says so and waits. No proxy metric is invented to force a decision.

**How this reconciles with section 6.3's ban, stated because an ungated second path was a real defect elsewhere in this plan.** Section 6.3 holds that nothing above tier 1 is scoped, scaffolded, or implemented until Checkpoint 11A. G7 does not shorten that. G7 produces *evidence*; Checkpoint 11B, if the owner creates it, produces *authorization*. Neither substitutes for the other, and a strong G7 result authorizes a proposal, not a build. The "near ceiling" test is deliberately excluded as a trigger: the ceiling is unknowable without the better system, so use the manual baseline and the observed variance in delight rate instead.

#### 6.5.8 If ever promoted: ensemble, never takeover

```text
verified candidate corpus
  → hard eligibility and suitability filters
  → deterministic quality and discovery signals
  → child-specific learned fit
  → caregiver fit where evidence exists
  → bounded exploration value
  → deterministic composition constraints
  → bag
```

The learned model may rerank eligible candidates or contribute a bounded score term. It may **not** create a candidate, make an ineligible book eligible, override age, safety, or suitability constraints, remove discovery guarantees, or write a factual explanation. Composition runs after it, deliberately, so discovery cannot be optimized away.

Classical per-child learning is **not** an `AIProvider` capability. `AIProvider` remains the boundary for language-model work — bounded classification, explanation rewording — whose privacy, latency, and truth risks differ in kind. A future `RankingPolicy` abstraction with deterministic and learned implementations is the likely shape, approved at its own design gate, with candidate generation, eligibility, and composition outside it so shadow evaluation, rollback, and deterministic fallback stay straightforward.

**Bounded active learning**, once and only once uncertainty is trustworthy: the exploration slot may prefer a candidate that both is likely to be good and would most resolve a live uncertainty about taste — for example distinguishing whether the child prefers deadpan, slapstick, rhyme, or absurdity. Constraints are absolute: at most one slot in a normal bag, every candidate already passes quality and suitability, exploration is never randomness, the caregiver can always reject it, and its performance is measured separately. Thompson sampling and related techniques may be evaluated later; none is approved here.

**Preference drift** is a real concern for a young child and is handled by evidence, not by adding decay now. Durable taste and current taste are already distinguished by the existing historical-versus-current interest split. Checkpoint 11A and later reviews test whether stale evidence is causing misses, and recency weighting is introduced only when outcomes show it helps. A strong preference from last year is not erased automatically, and a current obsession does not become a permanent identity.

#### 6.5.9 Conflicts with frozen contracts

Three, all resolved by deferral rather than by quietly editing an approved contract:

- **Section 5.4's taxonomy is frozen at the Checkpoint 5B schema gate** and contains no values for model snapshots, feature vectors, or training-evidence provenance. All of it defers to Checkpoint 11B's data-design gate. This is now the third deferred schema item, alongside Checkpoint 7D's dismiss states and its trip and outcome capture; they are tracked together so the queue stays visible rather than scattered.
- **Checkpoint 7B is blocked** pending the section 2.3 amendment, and the summary embeddings this section depends on are produced there. The real dependency chain is: section 2.3 amendment → Checkpoint 7B → embeddings exist → feature vectors become possible. Nothing here shortens it.
- **Checkpoint 7D's job is unchanged and becomes more important.** Its deterministic ranking stays; what this section adds is the reason its logging must be excellent. The feature contract determines what 7D has to capture, and **evidence not captured at 7D is gone forever** — a skipped book's reason cannot be reconstructed later. That, not the model, is why this section is written now rather than at Checkpoint 11.
...
**Two months is an operational guideline for when to look, not the trigger for the model-quality decision.** Gate G7 is triggered by evidence: the number of mature unique-work outcomes, the provenance quality of those labels, and whether both positive and negative information exist. Two months of light use can produce too few outcomes to decide anything, and saying so is a legitimate G7 result. This checkpoint evaluates the per-child content-based learner of section 6.5 as a distinct question from population-scale collaborative filtering, which stays out of scope; conflating them is the error section 6.3 corrects.
