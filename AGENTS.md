...
- **Stop and issue a field test rather than reasoning further.** Some questions cannot be answered from data the agent can reach, and guessing at them produces defects that look correct until a real check is run. Two separate, independently verified examples: the Open Library topic dictionary shipped with no juvenile filter and returned adult books for a child's topic, caught only by running the coverage matrix; and a catalog's facet vocabulary is not exposed in its feed, so it must be read off the interface rather than inferred. These are different systems and different causes, and neither is evidence for the other. When a real-world check would settle a question in under an hour, the field test wins over further analysis. Present it in this form and then stop that thread, continuing unrelated work:
  ...
  **One of these is a conversation, not a query, and it has been missed once already: talk to a children's librarian in person, once, early.** Ask what they actually use to pick read-alouds for a three-year-old, and whether staff-curated list data can be shared. The best available curation source may turn out to be a person rather than an endpoint, and this is the one field test that could change the corpus strategy rather than merely validate it. It is not satisfied by a librarian *reviewer persona*, which is an agent lens; this one requires a real visit.
